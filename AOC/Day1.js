function a(_input) {
  ...
  let numberSpells = [
    { spell: "one", number: 1 },
    { spell: "two", number: 2 },
    { spell: "three", number: 3 },
    { spell: "four", number: 4 },
    { spell: "five", number: 5 },
    { spell: "six", number: 6 },
    { spell: "seven", number: 7 },
    { spell: "eight", number: 8 },
    { spell: "nine", number: 9 },
  ];
  ...
  input.forEach((line, index) => {
    let indexes = [];
    numberSpells.forEach((s) => {
      indexes.push({
        index: line.indexOf(s.spell),
        lastIndex: line.lastIndexOf(s.spell),
        number: s.number,
        spell: s.spell,
      });
    });
    ...
    line = line.replace(indexes[0]?.spell, String(indexes[0]?.number));

    indexes = indexes.sort((a, b) =>
      a.lastIndex > b.lastIndex ? 1 : b.lastIndex > a.lastIndex ? -1 : 0
    );

    console.log(indexes);
    line = line.replaceAll(
      indexes[indexes.length - 1]?.spell,
      String(indexes[indexes.length - 1]?.number)
    );
    const numbers = line.split("").filter((e) => Number.isInteger(parseInt(e)));

    const first = numbers[0];
    const last = numbers[numbers.length - 1];

    solution += parseInt(first + "" + last);
    console.log(line);
  });
